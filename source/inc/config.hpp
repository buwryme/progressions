...
struct AppSettings {
    ...
    bool        systemd_configured = false;
    std::string update_interval    = "daily"; // daily | weekly | monthly
};
    ...
    static Config defaults() {
        ...
        c.commands = {
            { "pkg_upgrade", "Package Upgrade",
              ...
              "Update all packages including AUR", true, true },
            { "rebuild_initramfs", "Rebuild initramfs",
              "pkexec mkinitcpio -P",
              "drive-harddisk-symbolic",
              "Rebuild initramfs for all presets", true, false },
            { "flatpak_upgrade", "Flatpak Upgrade",
              "flatpak upgrade -y",
              "package-x-generic-symbolic",
              "Update all Flatpak applications", true, false },
            { "system_refresh", "System Refresh",
              "bash -c 'pkexec systemd-resolve --flush-caches 2>/dev/null; "
              "rm -rf ~/.cache/yay; "
              ...
              "Flush caches and refresh system state", true, false },
        };
        return c;
    }
...
private:
    ...
    static void set_settings_field(AppSettings& s,
        ...
        else if (key == "systemd_configured")     s.systemd_configured    = (val == "true");
        else if (key == "update_interval")        s.update_interval       = unquote(val);
    }
...
public:
    ...
    static Config load() {
        ...
        while (std::getline(f, line)) {
            ...
            if (line == "[[commands]]") {
                ...
                current = {}; in_cmd = true; in_settings = false;
    ...
    void save() const {
        ...
        ss << "# Progressions — System Update Manager\n\n"
           << "last_updated = \"" << escape_toml(last_updated) << "\"\n\n"
           << "[settings]\n"
           ...
           << "systemd_configured    = " << (settings.systemd_configured   ? "true" : "false") << "\n"
           << "update_interval       = \"" << escape_toml(settings.update_interval) << "\"\n\n";
